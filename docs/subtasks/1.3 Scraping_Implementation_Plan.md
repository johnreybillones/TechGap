...
# Scraper Timing (conservative rate limiting for single-IP safety)
...
├── Session 1:  Morning    (~30 min, ~50 jobs)
│   └── You sit at PC, solve any challenge once, let scraper run
├── Break:      1-2 hours
├── Session 2:  Afternoon  (~30 min, ~50 jobs)
│   └── Same pattern
├── Break:      2-3 hours
└── Session 3:  Evening    (~30 min, ~50 jobs)

Daily Target: ~150 unique job listings (conservative, safe)
Weekly Target: ~1,050 job listings
4-Week Target: ~4,200 job listings (+ ~1,000–1,500 from Adzuna seed = ~5,000–5,700 total)
```
