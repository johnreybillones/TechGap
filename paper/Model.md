...
* **Job Title & Position:** Essential for clustering (e.g., distinguishing between a "Frontend Developer" and a "Full Stack Developer").  
* **Job Description (Raw Text):** The source for semantic matching.  
* **Required Skills List:** Extracted keywords from job postings.  
* **Industry Demand Frequency:** How often a specific skill appears across thousands of scraped jobs (this is a key feature for the Ranking Engine).
...
* **Algorithms:** \* **Sentence-BERT (SBERT):** (Existing) Extracts semantic meaning.  
  * **Graph Convolutional Networks (GCN) or Node2Vec:** (New) Processes a "Skill Ontology" (e.g., a graph showing that *React* is a child of *JavaScript*).  
* **The Process:** For every skill in the curriculum and job post, the system generates a **Hybrid Embedding**. This vector contains both the text meaning and its position in the tech hierarchy.
...
* **Algorithms:** \* **LambdaMART (via LightGBM) or XGBoost:** A "Learning-to-Rank" (LTR) algorithm.  
  1. **TOPSIS (Multi-Criteria Decision Making):** Used to weight different factors.  
...
| :---- | :---- | :---- |
| **ADD** | **Skill Ontology (JSON/RDF)** | You must create or use a structured library (like ESCO or a custom JSON graph) that links related skills (e.g., FastAPI $\\rightarrow$ Python). |
| **MODIFY** | **Feature Vector** | Concatenate the 384-dim SBERT/Siamese output with the 64-dim Graph Embedding AND metadata vectors for Job Position and Curriculum Track. |
...
| **MODIFY** | **Data Collection** | Your web scraper must now not only collect skills but also **frequency counts** to feed the "Industry Demand Weight" for the ranking engine. |

### **Process Flow Summary:**

1. **Input:** Curriculum PDF (with Specialization Tag) \+ Job Post Text (with Title/Position separation).  
2. **Vectorize:** Use **SBERT \+ GCN** to get "Smart Vectors."  
